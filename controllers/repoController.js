...
async function fetchRepositoriesForCurrentUser(req, res) { 
  ...
  try {
    ...
    const user = await User.findOne({ username }).lean().populate("repositories");
    ...
    // Filter repositories based on ownership or visibility (avoid unnecessary data)
    let repositories = user.repositories;
    if (!isOwner) {
      console.log("User is NOT the owner. Filtering for public repositories...");
