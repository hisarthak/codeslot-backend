...
async function addModifiedOrLogs() {
  ...
  try {
    ...
    console.log("Checking inode match for:", relativePath, "-> Found:", matchedOldFile ? matchedOldFile.id : "No match");
    
    if (!matchedOldFile) {
      const potentialMatch = oldSnapshot[relativePath]; // Find match based on relative path
      console.log("Checking path match for:", relativePath, "-> Found:", potentialMatch ? "Yes" : "No");
    
      if (potentialMatch && potentialMatch.pull) {
        console.log("Matched based on path with pull: true -> Using this file.");
        matchedOldFile = potentialMatch;
      } else {
        console.log("Path match found but pull is false or not set -> Ignoring.");
      }
    }
